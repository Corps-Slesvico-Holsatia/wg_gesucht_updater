...
impl Session {
    ...
    async fn get_auth_data(&mut self, user_name: &str, password: &str) -> anyhow::Result<AuthData> {
        let php_session_id = self.get_php_session_id(user_name, password).await?;
        let (csrf_token, user_id) = self.get_csrf_token_and_user_id(&php_session_id).await?;
        Ok(AuthData::new(
            user_id,
    ...
    async fn get_php_session_id(
        ...
        cookies
            .get("PHPSESSID")
            .cloned()
            .ok_or_else(|| anyhow!("PHPSESSID not found"))
    }

    async fn get_csrf_token_and_user_id(
        &mut self,
        php_session_id: &str,
    ) -> anyhow::Result<(String, String)> {
        scrape_csrf_token_and_user_id(&Html::parse_document(&String::from_utf8(
            self.client
                .execute(self.build_offer_list_request(php_session_id)?)
                .await?
                .bytes()
                .await?
                .to_vec(),
    ...
    fn build_offer_list_request(&self, php_session_id: &str) -> reqwest::Result<Request> {
        self.client
            .get(OFFERS_LIST_URL)
            .header("User-Agent", &self.user_agent)
            .header("Cookies", format!("PHPSESSID={php_session_id}"))
            .timeout(self.timeout)
            .build()
    }
